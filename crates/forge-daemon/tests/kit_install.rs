...
const MCP_BIN: &str = "/Users/test/.featureforge/bin/forge-mcp";
...
fn kit_files(repo: &Path) -> BTreeMap<String, String> {
    ...
    for rel in [
        ...
        ".mcp.json",
        "CLAUDE.md",
        ".featureforge/hooks/forward.sh",
        ".featureforge/.gitignore",
    ] {
...
fn install_twice_is_idempotent() {
    ...
    for event in ["PostToolUse", "Stop", "SessionStart"] {
        ...
        let ours = groups
            .iter()
            .filter(|g| g.to_string().contains(".featureforge/hooks/forward.sh"))
            .count();
        assert_eq!(ours, 1, "exactly one featureforge group for {event}");
    }
}
...
fn settings_merge_preserves_user_hooks_and_keys() {
    ...
    assert_eq!(post.len(), 2, "user group + featureforge group");
    ...
    assert_eq!(
        ...
        "\"$CLAUDE_PROJECT_DIR\"/.featureforge/hooks/forward.sh"
    );
    assert!(merged["hooks"]["Stop"].is_array());
    assert!(merged["hooks"]["SessionStart"].is_array());

    // Re-install: still exactly one featureforge PostToolUse group.
    install_kit(repo, Path::new(MCP_BIN)).unwrap();
...
fn mcp_json_merge_preserves_other_servers() {
    ...
    assert_eq!(merged["mcpServers"]["featureforge"]["command"], MCP_BIN);
    assert_eq!(merged["mcpServers"]["featureforge"]["args"], serde_json::json!([]));
}
...
fn claude_md_section_appends_then_repairs() {
    ...
    assert!(text.contains("<!-- featureforge:start -->"));
    assert!(text.contains("<!-- featureforge:end -->"));
...
fn gitignore_ignores_runtime() {
    ...
    let text = std::fs::read_to_string(tmp.path().join(".featureforge/.gitignore")).unwrap();
...
fn forward_sh_is_valid_posix_sh_and_executable() {
    let tmp = tempfile::tempdir().unwrap();
    install_kit(tmp.path(), Path::new(MCP_BIN)).unwrap();
    let script = tmp.path().join(".featureforge/hooks/forward.sh");
